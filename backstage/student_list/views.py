...
class LearningTime(APIView):
	...
	def get(self,request,*args,**kwargs):
		...
		try:
			...
			time = "2018-08-08" #self.request.GET.get("time", "")  # 时间
			...
			for user in user_list:
				...
				watchrecords = WatchRecord.objects.filter(user=user)
				if watchrecords.exists():
					watchrecord = watchrecords.first()
					# time=watchrecord.create_time.strftime("%Y-%m-%d")
					# section=watchrecord.video.section.title


					print("wa",watchrecord)
