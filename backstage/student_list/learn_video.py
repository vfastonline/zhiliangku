...
class LearnVideo(APIView):
	...
	def get(self, request, *args, **kwargs):
		...
		try:
			...
			assess_list = UnlockVideo.objects.filter(**param).order_by("video__id").annotate(
				sum_assess=Sum("times")).values(
				"sum_assess", "video__name", "video__section__title", "video__section__course__name",
				"video__section__course__project__name"
			)
			assess_times = []
			for assess in assess_list:
				assess_node = assess["video__section__course__project__name"] + assess[
					"video__section__course__name"] + assess["video__section__title"] + assess["video__name"]
				sum_times = assess["sum_assess"]

				result = {
					"exercise_node": assess_node,  # 练习视频节点
					"sum_times": sum_times,  # 练习次数
				}
				assess_times.append(result)

			data = {"video_time": video_time, "exercise_times": exercise_times, "assess_times": assess_times}
		except:
			traceback.print_exc()
