...
from applications.custom_user.models import CustomUserProject
...
#                     detail["courses_count"] = 0#sum([coursecategory.courses.all().count() for coursecategory in CourseCategory.objects.filter(path_stage__path=path_obj)])
#
#                     courses_counts =0 #[one.courses.all().count() for one in CourseCategory.objects.filter(path_stage__in=path_obj.PathStage.all())]
...
def user_path_summarize(custom_user_id, project_obj):
    """汇总项目学习情况
    :param custom_user_id:用户ID
    :param project_obj:项目
    :return:
    """
    summarize_dict = {
    ...
    try:
        # 是否参与该路径
        customuserpaths = CustomUserProject.objects.filter(custom_user__id=custom_user_id, Project=project_obj)
...
class ParticipateProject(View):
    """参与该学习该项目"""

    def post(self, request, *args, **kwargs):
        result_dict = {"err": 0, "msg": "成功参与该学习项目"}
        try:
            ...
            customuserprojects = CustomUserProject.objects.filter(**filter_param)
            if not customuserprojects.exists():
                obj = CustomUserProject.objects.create(**filter_param)
                if not obj:
                    result_dict["err"] = 1
                    result_dict["msg"] = "参与学习项目失败"
                    return
        except:
            traceback.print_exc()
            logging.getLogger().error(traceback.format_exc())
