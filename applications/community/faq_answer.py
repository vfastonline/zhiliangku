...
@class_view_decorator(user_login_required)
class DelFaqAnswer(View):
	"""删除-答案"""

	def __init__(self):
		super(DelFaqAnswer, self).__init__()
	...
	def post(self, request, *args, **kwargs):
		try:
			param_dict = json.loads(request.body)
			custom_user_id = str_to_int(kwargs.get('uid', 0))  # 用户ID
			faqanswer_id = str_to_int(param_dict.get('faqanswer_id', 0))  # 回答ID

			faqanswers = FaqAnswer.objects.filter(user__id=custom_user_id, id=faqanswer_id)
			if faqanswers.exists():
				deleted, _rows_count = faqanswers.delete()
				self.result_dict["data"] = deleted
			else:
				self.result_dict["msg"] = "只有回答者才能删除"
		except:
			traceback.print_exc()
			logging.getLogger().error(traceback.format_exc())
...
@class_view_decorator(user_login_required)
class EditFaqAnswer(View):
	"""编辑-答案"""

	def __init__(self):
		super(EditFaqAnswer, self).__init__()
	...
	def post(self, request, *args, **kwargs):
		try:
			...
			custom_user_id = str_to_int(kwargs.get('uid', 0))  # 用户ID
			faqanswer_id = str_to_int(param_dict.get('faqanswer_id', 0))  # 回答ID
			answer = param_dict.get('answer', "")  # 回答
			...
			if faqanswers.exists():
				...
				update_count = faqanswers.update(**update_param)
				if update_count:
					self.result_dict["data"] = update_param
			else:
				self.result_dict["msg"] = "只有回答者才能编辑"
		except:
			traceback.print_exc()
			logging.getLogger().error(traceback.format_exc())
			self.result_dict["msg"] = traceback.format_exc()
