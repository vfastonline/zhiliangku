...
class GetCustomUserAvatar(View):
	"""获取用户头像信息"""

	def __init__(self):
		super(GetCustomUserAvatar, self).__init__()
		self.result_dict = {
			"err": 0,
			"msg": "success",
			"data": "",
	...
	def get(self, request, *args, **kwargs):
		try:
			...
			self.result_dict["data"] = CustomUser.objects.get(id=custom_user_id).avatar.url
		except:
			self.result_dict["err"] = 1
			self.result_dict["msg"] = "没有头像信息"
			traceback.print_exc()
