<template>
  <div class="project-header "
    ...
    <div class="ph-container  inmiddle clearfix ">
      ...
      <div class="main">
        <div class="inner">
          <div v-if="['videoHeader','liveHeader'].indexOf(type)+1"
               class="ph-content white">
          ...
          <div v-if="!type"
            ...
            <span class="ph-tag pointer">
              <a href="/tracks/path/list/">
                <el-badge value="free!"
                          class="project-header-free">
                  <span> 高薪就业班 </span>
                </el-badge>
              </a>
            </span>
            <span class="ph-tag pointer">
              <a href="/community/faq/list/"> 社区</a>
            </span>
      ...
      <div class="left">
        <!-- <img v-if="!type"
             class='ph-logo pointer'
             @click="goindex()"
             src='../../assets/img/icons/Logo.png'
             alt="">
        <img v-if="['videoHeader','liveHeader'].indexOf(type)+1"
             @click="showVideoList()"
             class='ph-expend-button pointer'
             src='../../assets/img/icons/视频播放+习题图标/视频播放_汉堡按钮.svg'
             alt=""> -->
      </div>
      <div class="rightbar">
        <!--<el-button @click="changShow()" class="ph-button" type="primary" :style="buttonStyle" round>岗位匹配</el-button>-->
        <span v-on:mouseenter="changeUsershow1()"
              v-on:mouseleave="changeUsershow2()"
              class="user-info  font18pl3a3c50"
              :class="{'white':['videoHeader','liveHeader'].indexOf(type)+1}">
          <!-- <img v-if="is_login"
          ...
          <span v-if="!is_login"
                class="pointer"
                @click="myDispatch('open','loginActive')">登录 |</span>
          <span v-if="!is_login"
                class="pointer"
                @click="myDispatch('open','logupActive')">注册</span>
        </span>
...
import Login from '../../components/02_login/01_login.vue'
// import postMatch from '../../components/home/postMatch'
import userMune from './02_userIconMune.vue'
