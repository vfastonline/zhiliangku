<template>
  <div class="login-box">
    <!-- 极有可能是bug的地方：回车键的体验 -->
    <el-dialog @close="modalClose()"
               :show-close="false"
               :visible.sync="centerDialogVisible">
      <!-- 1.login模块 -->
      <div v-show="allshow.loginActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">登录</div>
      <el-form v-show="allshow.loginActive"
               :model="ruleForm1"
               status-icon
               :rules="rules2"
               ref="myform1">
        <el-form-item label="请输入邮箱/手机号注册"
                      prop="age">
          <el-input v-model="ruleForm1.age"></el-input>
        </el-form-item>
        <el-form-item label="密码"
                      prop="pass">
          <el-input @keyup.enter.native="submitForm('myform1','loginFun',ruleForm1)"
                    type="password"
        ...
        <div class="clearfix">
          ...
          <div class="icons floatr">
            <a :href="qqBase64Url">
              <!-- <img class="longin-bottom-icons pointer"
                   src="../../assets/img/icons/QQ.svg"
                   alt=""> -->
            </a>
            <a :href="wxBase64Url">
              <!-- <img class="longin-bottom-icons pointer"
      ...
      <div v-show="allshow.loginActive"
           slot="footer">
        <div @click="changeModal('logupActive')"
             class="login-bottom-button incenter fontcenter pointer font20plffffff">
          <span>创建账号</span>
        </div>
      </div>
      <!--2. logup模块 -->
      <div v-show="allshow.logupActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">账号创建</div>
      <el-form v-show="allshow.logupActive"
               :model="ruleForm2"
               status-icon
               :rules="rules2"
               ref="myform2">
        ...
        <el-form-item label="6-16位密码（区分大小写）"
                      prop="pass">
          <el-input @keyup.enter.native="submitForm('myform2','logupType',ruleForm2,changeModal,'verifyEmailActive')"
                    type="password"
                    v-model="ruleForm2.pass"
                    auto-complete="off"></el-input>
        </el-form-item>
        <el-button @click="submitForm('myform2','logupType',ruleForm2,changeModal,'verifyEmailActive')"
                   :class="['marginbottom24','login-commen-container-button','font20plffffff','fontcenter','incenter','pointer']">创建</el-button>
        <div class="clearfix">
          <div class="floatl font14pl5A646E">其他登录方式:</div>
          <div class="icons floatr">
            <a :href="qqBase64Url">
      ...
      <div v-show="allshow.logupActive"
           slot="footer">
        <div @click="changeModal('loginActive')"
             class="login-bottom-button incenter fontcenter pointer font20plffffff">
          <span>已有账号，去登录</span>
        </div>
      </div>
      <!--3. getpassword模块 -->
      <div v-show="allshow.getPasswordActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">找回密码</div>
      <el-form v-show="allshow.getPasswordActive"
               :model="ruleForm3"
               status-icon
               :rules="rules2"
               ref="myform3">
        <el-form-item label="请输入邮箱/手机号"
                      prop="age">
          <el-input @keydown.enter.native="submitForm('myform3','getPassType',ruleForm3,changeModal,'verificationCodeActive')"
                    v-model="ruleForm3.age"></el-input>
        </el-form-item>
        <el-button @click="submitForm('myform3','getPassType',ruleForm3,changeModal,'verificationCodeActive')"
                   :class="['marginbottom8','login-commen-container-button','font20plffffff','fontcenter','incenter','pointer']">发送验证码</el-button>
      </el-form>
      <!--4. verificationCode 验证模块 -->
      <div v-show="allshow.verificationCodeActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">找回密码</div>
      <el-form v-show="allshow.verificationCodeActive"
               :model="ruleForm4"
               status-icon
               :rules="rules2"
               ref="myform4">
        ...
        <el-form-item label="输入新的密码"
                      prop="pass">
          <el-input @keydown.enter.native="submitForm('myform4','modifyPass',ruleForm4,ruleForm3)"
                    type="password"
                    v-model="ruleForm4.pass"
                    auto-complete="off"></el-input>
        </el-form-item>
        <el-button @click="submitForm('myform4','modifyPass',ruleForm4,ruleForm3)"
                   :class="['marginbottom8','login-commen-container-button','font20plffffff','fontcenter','incenter','pointer']">提交</el-button>
        <div class="incenter fontcenter">
          <span @click="changeModal('getPasswordActive')"
                class="font14pl7c7e8c pointer">
            返回修改邮箱手机号
          </span>
        </div>
      </el-form>
      <!--5. verifyCode 创建账号-->
      <div v-show="allshow.verifyCodeActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">账号创建</div>
      <el-form v-show="allshow.verifyCodeActive"
               :model="ruleForm5"
               status-icon
               :rules="rules2"
               ref="myform5">
        <el-form-item label="请输入短信验证码">
          <el-input @keydown.enter.native="verifyPhoneCode()"
                    v-model="ruleForm5.age"></el-input>
        </el-form-item>
        <el-button @click="verifyPhoneCode()"
                   :class="['marginbottom8','login-commen-container-button','font20plffffff','fontcenter','incenter','pointer']">登录</el-button>
        <div class="incenter fontcenter">
          <span @click="changeModal('logupActive')"
                class="font14pl7c7e8c pointer">
            返回修改手机号
          </span>
        </div>
      ...
      <!--6. success模块 -->
      <div v-show="allshow.verifyEmailActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">账号创建</div>
      <div v-show="allshow.verifyEmailActive"
           class="logup-success-body">
        <!-- <img class="marginbottom24 incenter block"
             src="../../assets/img/icons/注册弹窗 邮箱激活弹窗_对勾.svg"
             alt=""> -->
      ...
      <el-button @click="goEmailHome(ruleForm2.age)"
                 v-show="allshow.verifyEmailActive"
                 :class="['login-commen-container-button','font20plffffff','fontcenter','incenter','pointer']">去邮箱验证</el-button>
      <!--7. success 模块 -->
      <div v-show="allshow.goEmailActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">账号创建</div>
      <div v-show="allshow.goEmailActive"
           class="logup-success-body">
        <!-- <img class="marginbottom24 incenter block"
             src="../../assets/img/icons/找回密码弹窗_邮件找回_邮件.svg"
             alt=""> -->
        <div class="fontcenter marginbottom24">邮件已发至
          <!-- 这里确实是有bug存在的，需要修改 -->
      ...
      <el-button @click="goEmailHome(ruleForm3.age)"
                 v-show="allshow.goEmailActive"
                 :class="['login-commen-container-button','font20plffffff','fontcenter','incenter','pointer']">去邮箱</el-button>
      <!-- 8.再次验证邮箱模块 -->
      <div v-show="allshow.emailVerifyAginActive"
           slot="title"
           class="fontcenter font18plffffff marginbottom8">账号创建</div>
      <div v-show="allshow.emailVerifyAginActive"
           class="logup-success-body">
        <!-- <img class="marginbottom24 incenter block"
             src="../../assets/img/icons/Email叹号.svg"
             alt=""> -->
      ...
      <div v-show="allshow.emailVerifyAginActive"
           class="login-middle-button-container incenter">
        <el-button @click="goEmailHome(keyEmail)"
                   :class="['login-middle-button','font20plffffff','fontcenter','incenter','pointer']">去邮箱验证</el-button>
        <el-button @click="getEmailVerify(keyEmail)"
                   :class="['login-middle-button','font20plffffff','fontcenter','incenter','pointer','floatr']">发送验证码</el-button>
      </div>
    </el-dialog>
  </div>
</template>
